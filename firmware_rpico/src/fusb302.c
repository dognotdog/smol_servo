...
int fusb302_init(i2c_inst_t* i2c) {
    ...
    if (!result) {
        printf("FUSB302 device_id read failed.\n");
        return result;
    }

    printf("FUSB302 device_id 0x%02X\n", device_id);
...
int fusb302_write_reg(fusb302_reg_addr_t reg, uint8_t value) {
    ...
    if (result != 2)
        printf("ERR fusb302_write_reg failed: i2c_write_blocking_until()=%i.\n", result);
    return result;
}
int fusb302_read_reg(fusb302_reg_addr_t reg, uint8_t* value) {
    ...
    if (result != 1) {
        printf("ERR fusb302_read_reg failed: i2c_write_blocking_until()=%i.\n", result);
        return result;
    }
